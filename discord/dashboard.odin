...
render_dashboard :: proc(client: ^Client) {
       ...
       ack_status := client.received_ack
       ...
       fmt.eprintfln("  %-18s %d", "Users:", total_members)
       ...
       fmt.eprintfln("  %-18s %s", "Gateway Status:", ack_status ? "OK" : "WAITING ACK")
...
_compute_latency_stats :: proc(client: ^Client) -> (f64, f64) {
       ...
       defer sync.unlock(&client.latency_mutex)
